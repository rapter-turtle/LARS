...
for i = 2:N
    if mod(i, control_update_steps) == 0 
        ...
        %% DOB
        x_estim_k = x_estim(i-1) + control_update_dt*(ux_log(i-1) + x_dis_estim(i-1));
        y_estim_k = y_estim(i-1) + control_update_dt*(uy_log(i-1) + y_dis_estim(i-1));
        x_error_k = x_estim_k - x(i-1);
        y_error_k = y_estim_k - y(i-1); 
    ...
    x_dot(i) = x_dot(i-1) + dt*(ux + wx(i-1));
    y_dot(i) = y_dot(i-1) + dt*(uy + wy(i-1));
...
subplot(2,3,6);
plot(t, x_dis_estim, 'm', 'LineWidth', 2); hold on;
plot(t, y_dis_estim, 'm', 'LineWidth', 2);
