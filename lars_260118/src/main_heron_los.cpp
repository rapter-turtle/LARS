...
private:
    ...
    bool loadParamsFromTxt(const std::string& path)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            try {
                ...
                else if (key == "dock_start_distance")     set_if(key, dock_start_distance, v);
                else if (key == "mode_change_distance")     set_if(key, mode_change_distance, v);
    ...
    void ekf_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
    {
        if (only_xypsi)
        {
            ...
            const double psi_meas_raw = msg->data[2];
            ...
            if (!pose_time_init_)
            {
                ...
                last_pose_time_ = now;
                dt = dt_control_;   // 첫 샘플은 컨트롤 주기로 가정
            }
            else
            {
            ...
            // --- psi unwrap (기준: 이전 필터 psi) ---
            const double psi_unwrapped = yaw_unwrap(psi_meas_raw, psi_lpf_);

            // --- LPF 업데이트 ---
            const double a = lpf_alpha_from_fc(dt, pose_lpf_fc_);
            x_lpf_  = lpf_update(x_meas,        x_lpf_,  a);
            y_lpf_  = lpf_update(y_meas,        y_lpf_,  a);
            psi_lpf_= lpf_update(psi_unwrapped, psi_lpf_, a);

            // --- EKF step (추천: 필터된 pose로 u,v,r 안정화) ---
            uvr_ekf_.step(x_lpf_, y_lpf_, psi_lpf_, dt);

            // --- MPC/LOS에서 쓰는 상태는 필터된 pose 사용 ---
            x_   = x_lpf_;
            y_   = y_lpf_;
            yaw_ = psi_lpf_;
            ...
            // RCLCPP_INFO(this->get_logger(),
            // "[LOS] lpf x=%.2f, real x=%.2f", x_lpf_, x_meas);
            // RCLCPP_INFO(this->get_logger(),
            // "[LOS] lpf x - real x=%.2f", x_lpf_- x_meas);
        }
        else
        {
    ...
    void run()
    {
        ...
        double x_local = x_ - offset_x_;
        double y_local = y_ - offset_y_;
        ...
        if (!alos_init_ || second_phase ||  target_change> 0.5)  // 0.5m는 예시 (원하면 조절)
        {
            ...
            if (!alos_init_){
                ...
                dp_x = x_local;
                start_x = x_local - 10;
                start_y = y_ - offset_y_;
                last_target_x_ = x_t + 10;
                last_target_y_ = y_ - offset_y_;
                RCLCPP_INFO(this->get_logger(),
                "[LOS] ayyyyyyyyyyyyyyyyy");
            }
        ...
        if (CD_y <= y_ - offset_y_ + mode_change_distance ){
        ...
        if (stop_phase){
            if (start_y <= CD_y - dock_start_distance){
            start_y += lateral_speed;
            last_target_y_ += lateral_speed; 
            }
        }

        ////////////// ALOS guidance law //////////////
        // 4) 경로 방위각 pi_h (start -> target)
        const double seg_dx = last_target_x_ - start_x;
        const double seg_dy = last_target_y_ - start_y;
        ...
        double psi_d = uvr_filter::wrapToPi(pi_h - beta - std::atan2(y_ep, Delta) - yaw_);
        ...
        if (stop_phase && (start_y >= CD_y - dock_start_distance)){
            dist = CD_x - (x_ - offset_x_);
        }
        else{
            dist = dp_x - (x_ - offset_x_);
        }
        // RCLCPP_INFO(this->get_logger(),
        // "[LOS] dist=%.2f", dist);
        // RCLCPP_INFO(this->get_logger(),
...
private:
    ...
    double lateral_speed = 0.01;
    double mode_change_distance = 10.0;
    double dock_start_distance = 1.0;

    // =====================
    // Pose LPF (always ON)
    // =====================
    bool pose_lpf_init_ = false;
    double x_lpf_ = 0.0;
    double y_lpf_ = 0.0;
    double psi_lpf_ = 0.0;

    double pose_lpf_fc_ = 0.45;   // [Hz] cutoff
