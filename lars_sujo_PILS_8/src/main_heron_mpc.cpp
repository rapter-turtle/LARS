...
private:
    ...
    static inline double lpf_alpha_from_fc(double dt, double fc_hz)
    {
        ...
        if (fc_hz <= 0.0) return 1.0; // fc<=0이면 필터 없이 통과
        const double tau = 1.0 / (2.0 * M_PI * fc_hz);
        return dt / (tau + dt);       // alpha in (0,1)
    }
    ...
    void ekf_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
    {
        if (only_xypsi)
        {
            ...
            // dt 계산 (초기에는 dt를 "가정값"으로 사용)
            double dt = 0.0;
            if (!pose_time_init_)
            {
                pose_time_init_ = true;
                last_pose_time_ = now;
                dt = dt_control_; // 초기 1회는 컨트롤 주기 정도로 잡아줌
            }
            else
            {
            ...
            // -------- 축별 alpha --------
            const double ax   = lpf_alpha_from_fc(dt, pose_lpf_fc_x_);
            const double ay   = lpf_alpha_from_fc(dt, pose_lpf_fc_y_);
            const double apsi = lpf_alpha_from_fc(dt, pose_lpf_fc_psi_);
            ...
            // -------- EKF step (추천: 필터된 pose로 u,v,r 더 안정적) --------
    ...
    void run()
    {
        ...
        for (int j = 0; j <= N_; ++j)
        {
            ...
            if (!eight_traj_check){
                xr = wpt_x + j*dt_control_*MS_speed*cos(MS_psi);
                yr = wpt_y + j*dt_control_*MS_speed*sin(MS_psi);
                pr = wpt_psi;
            }
            else{
                int k = std::min(ref_idx_ + j * ref_step_, (int)reference_.size() - 1);

                xr = reference_[k].x - offset_x_;     // RefPoint 멤버명에 맞게 수정
                yr = reference_[k].y - offset_y_;
                pr = reference_[k].yaw;  

             
            }

            if (eight_traj_check && !reference_.empty())
            {
                ref_idx_ += ref_step_;
                if (ref_idx_ >= (int)reference_.size()) {
                    ref_idx_ = 0; // 끝에서 고정
                    // 또는 반복시키고 싶으면: ref_idx_ = 0;
                }
            }
        ...
        // ==========================================
        // DOB 적용 타이밍: 시작 후 5초 이후부터 적용
        // ==========================================
        const double t_from_start = (now - start_time_).seconds();
        ...
        if (t_from_start >= 5.0)
        {
            dob_u = dob_state_.param_filtered[0];
            dob_v = dob_state_.param_filtered[1];
            dob_r = dob_state_.param_filtered[2];
        }

        // double dob_u = dob_state_.param_filtered[0];
        // double dob_v = dob_state_.param_filtered[1];
        // double dob_r = dob_state_.param_filtered[2];
    ...
    void generateReference()
    {
        reference_ = ReferenceGenerator::generateFigureEight(
            10000.0, ref_dt_, ref_x_, ref_y_, theta_, A_, B_, C_);
...
private:
    ...
    // cutoff [Hz] (축별로 다르게)
    double pose_lpf_fc_x_   = 10.0;  // 예: x 1 Hz
    double pose_lpf_fc_y_   = 10.0;  // 예: y 1 Hz
    double pose_lpf_fc_psi_ = 10.0;  // 예: psi 0.5 Hz

    double xr;
    double yr;
    double pr;

    bool eight_traj_check;
