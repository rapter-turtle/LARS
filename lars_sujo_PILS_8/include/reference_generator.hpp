...
public:
        ...
        double C = 60.0)
    {
        ...
        for (int i = 0; i < N; ++i) t_vec[i] = i * dt;
        ...
        // arc length increment (distance between samples; central-diff distance)
        std::vector<double> ds(N, 0.0);
        for (int i = 1; i < N - 1; i++) {
            double dx = x[i + 1] - x[i - 1];
            double dy = y[i + 1] - y[i - 1];
            ds[i] = 0.5 * std::sqrt(dx * dx + dy * dy);
        }
        ds[0]     = ds[1];
        ds[N - 1] = ds[N - 2];
        ...
        // map s -> t via interpolation (CORRECT)
        std::vector<double> t_uniform(N);
        for (int i = 0; i < N; i++)
            t_uniform[i] = interp1D(s, s_uniform[i], t_vec);
    
        // build uniform-arc reference
        std::vector<RefPoint> ref;
        ref.reserve(N);
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            // derivatives w.r.t time (for yaw/u)
            double dxu = (A / C) * std::cos(t / C);
            double dyu = (B / C) * (std::cos(t / C)*std::cos(t / C) -
                                    std::sin(t / C)*std::sin(t / C));
    
            double yaw_raw = std::atan2(dyu, dxu);
            double yaw_u = yaw_raw;
    
            if (!yaw_init) { yaw_prev = yaw_raw; yaw_init = true; }
            yaw_u = unwrapAngle(yaw_prev, yaw_raw);
            yaw_prev = yaw_u;
    
            double u = std::sqrt(dxu*dxu + dyu*dyu);
            ...
            // r: yaw rate (finite difference on uniform samples)
            if (i == 0) rp.r = 0.0;
            else        rp.r = (rp.yaw - ref.back().yaw) / dt;
    
            ref.push_back(rp);
        }
    
        // smooth first r
        if (ref.size() >= 2) ref[0].r = ref[1].r;
