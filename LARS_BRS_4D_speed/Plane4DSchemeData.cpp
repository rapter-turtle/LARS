...
bool Plane4DSchemeData::hamFunc(
	...
	for (size_t i = 0; i<length; ++i) {
		...
		FLOAT_TYPE dx = 0.4;
		FLOAT_TYPE dy = 0.15;
		FLOAT_TYPE dpsi = 0.15;
		FLOAT_TYPE alpha_max = 0.4;
			
		// hamValue[i] = -((V0*std::cos(xs2_i)) * deriv0_i - dx*HjiFabs(deriv0_i)
		// + (V0*std::sin(xs2_i)) * deriv1_i - dy*HjiFabs(deriv1_i)
		// + xs3_i*deriv2_i - dpsi*HjiFabs(deriv2_i)
		// + alpha_max*HjiFabs(deriv3_i));	

		// hamValue[i] = -((Kp*std::sqrt(xs0_i*xs0_i + xs1_i*xs1_i)*std::cos(xs2_i)) * deriv0_i - dx*HjiFabs(deriv0_i)
		// + (Kp*std::sqrt(xs0_i*xs0_i + xs1_i*xs1_i)*std::sin(xs2_i)) * deriv1_i - dy*HjiFabs(deriv1_i)
		// + xs3_i*deriv2_i - dpsi*HjiFabs(deriv2_i)
		// + alpha_max*HjiFabs(deriv3_i));	

		hamValue[i] = -((-Kp*xs0_i*std::cos(xs2_i)) * deriv0_i - dx*HjiFabs(deriv0_i)
		+ (Kp*xs0_i*std::sin(xs2_i)) * deriv1_i - dy*HjiFabs(deriv1_i)
		+ xs3_i*deriv2_i - dpsi*HjiFabs(deriv2_i)
		+ alpha_max*HjiFabs(deriv3_i));				
...
bool Plane4DSchemeData::partialFunc(
	...
	case 0:
	{
		...
		const beacls::FloatVec &xs2 = hji_grid->get_xs(2);
		for (size_t i = 0; i<length; ++i) {
			// alphas[i] = HjiFabs(V0*std::cos(xs2[i+ begin_index]) - dx);
			// alphas[i] = HjiFabs(Kp*std::sqrt(xs0[i+ begin_index]*xs0[i+ begin_index] + xs1[i+ begin_index]*xs1[i+ begin_index])*std::cos(xs2[i+ begin_index]) - dx);
			alphas[i] = HjiFabs(-Kp*xs0[i+ begin_index]*std::cos(xs2[i+ begin_index]) - dx);
	...
	case 1:
	{
		...
		const beacls::FloatVec &xs2 = hji_grid->get_xs(2);
		for (size_t i = 0; i<length; ++i) {
			// alphas[i] = HjiFabs(V0*std::sin(xs2[i+ begin_index]) - dy);
			alphas[i] = HjiFabs(-Kp*xs0[i+ begin_index]*std::sin(xs2[i+ begin_index]) - dy);
		}
	}
	break;
	case 2:
	{
		...
		const beacls::FloatVec &xs3 = hji_grid->get_xs(3);
		for (size_t i = 0; i<length; ++i) {
			alphas[i] = HjiFabs(xs3[begin_index + i] - dpsi);
		}
	}
	break;
	case 3:
