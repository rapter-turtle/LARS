...
def export_heron_model():
    ...
    # Dynamics (expl)
    f_expl = vertcat(
        -stream_speed + u*cos(psi) - v*sin(psi),
        u*sin(psi) + v*cos(psi),
        r,
...
def setup_trajectory_tracking(x0, N_horizon, Tf):
    ...
    Q_mat = 2*np.diag([1e3, 1e3, 1e-2, 1e3, 0.0, 0.0, 1e-1, 1e0])
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-100,-10.0])
    ocp.constraints.ubu = np.array([+100,+10.0])
