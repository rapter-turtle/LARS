...
class ShipSimulator(Node):
    ...
    def ship_dynamics(self, state, control, dt):
        ...
        # Dynamics
        u_dot = (- Xu*u - Xuu * np.sqrt(u * u + eps) * u + alpha1*F_cmd*np.cos(alpha2*delta))/M
        v_dot = (-Yv*v - Yr*r - Yvv * np.sqrt(v* v + eps) * v + alpha3*F_cmd*np.sin(alpha2*delta))/M
        r_dot = (- Nr*r - Nv*v - Nrr * np.sqrt(r * r + eps) * r - alpha4*F_cmd*np.sin(alpha2*delta))/I
        # print(u, v, r, F_cmd)
