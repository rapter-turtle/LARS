...
class ShipSimulator(Node):
    def __init__(self):
        ...
        # ROS2 I/O
        self.state_pub = self.create_publisher(Float64MultiArray, '/USVControlType', 10)
        ...
        self.ship_state[0] = 0.0#(289577.66 + 291591.05) * 0.5 - 25.0# X
        self.ship_state[1] = 0.0#(4117065.30 + 4118523.52) * 0.5 - 30.0 # Y
        ...
        self.MS_x = 10.0#(289577.66 + 291591.05) * 0.5
        self.MS_y = 10.0#(4117065.30 + 4118523.52) * 0.5

        self.MS_psi = 0 * np.pi / 180.0
        self.MS_speed = 0.0

        self.CD_side = 13.0
        self.CD_x = self.MS_x + self.CD_side*np.sin(self.MS_psi)
        self.CD_y = self.MS_y - self.CD_side*np.cos(self.MS_psi)
        self.CD_psi = self.MS_psi

        self.stream_speed = 2.57
    ...
    def ship_dynamics(self, state, control, dt):
        ...
        # u_dis = np.sin(0.01*self.tt)*0.2*np.cos(psi)
        # v_dis = np.sin(0.01*self.tt)*0.2*np.sin(psi) - 0.01*np.sin(0.02*self.tt) 
        # r_dis = np.cos(0.02*self.tt)*0.03
        self.real_stream = 0.0#self.stream_speed + np.sin(0.01*self.tt)*0.5
