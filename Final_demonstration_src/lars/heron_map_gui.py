...
# 460288.19840289274 3883806.954683061
#Jeongok
# x_actual_min = 460288.198 - 500.0
# x_actual_max = 460288.198 + 500.0
# y_actual_min = 3883806.95 - 1000.0
# y_actual_max = 3883806.95 + 1000.0
# x_center = 460288.198
# y_center = 3883806.95
...
class SensorFusionEKF(Node):
    ...
    def ekf_callback(self, msg):  # - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        ...
        # self.x, self.y, zone, letter = utm.from_latlon(msg.data[0], msg.data[1])
        self.x, self.y = msg.data[0], msg.data[1]
        
        # print("raw lat/lon? :", msg.data[0], msg.data[1])
        # x_utm, y_utm, zone, letter = utm.from_latlon(msg.data[0], msg.data[1])
        # print("converted utm:", x_utm, y_utm, zone, letter)
        self.p = msg.data[2] - 0.5*3.141592
        self.u = msg.data[3]
        self.v = msg.data[4]
        self.r = msg.data[5]
        print(self.x)

        self.x_sensor = 0.0#msg.data[6]
        self.y_sensor = 0.0#msg.data[7]
        self.p_sensor = 0.0#-msg.data[8]
        self.u_sensor = 0.0#msg.data[9]
        self.v_sensor = 0.0#msg.data[10]
        self.r_sensor = 0.0#msg.data[11]


        #   ros.data[6]  = s.MSHIP_x();
        #   ros.data[7]  = s.MSHIP_y();
        #   ros.data[8]  = s.MSHIP_h();
        #   ros.data[9]  = s.Craddle_x1();
        #   ros.data[10]  = s.Craddle_y1();
        #   ros.data[11]  = s.Craddle_x2();
        #   ros.data[12]  = s.Craddle_y2();
        #   ros.data[13] = s.Craddle_x3();
        #   ros.data[14] = s.Craddle_y3();
        #   ros.data[15] = s.Craddle_x4();
        #   ros.data[16] = s.Craddle_y4();
        #   ros.data[17] = s.Craddle_h();

        self.MS_x, self.MS_y, zone, letter = utm.from_latlon(msg.data[6], msg.data[7])

        cdx1, cdy1, zone, letter = utm.from_latlon(msg.data[9], msg.data[10])
        cdx2, cdy2, zone, letter = utm.from_latlon(msg.data[11], msg.data[12])
        cdx3, cdy3, zone, letter = utm.from_latlon(msg.data[13], msg.data[14])
        cdx4, cdy4, zone, letter = utm.from_latlon(msg.data[15], msg.data[16])
        # cdx1, cdy1 = msg.data[9], msg.data[10]
        # cdx2, cdy2 = msg.data[11], msg.data[12]
        # cdx3, cdy3 = msg.data[13], msg.data[14]
        # cdx4, cdy4 = msg.data[15], msg.data[16]
        

        self.CD_x = (cdx1 + cdx2 + cdx3 + cdx4)/4
        self.CD_y = (cdy1 + cdy2 + cdy3 + cdy4)/4
        self.CD_psi = -msg.data[8] + 0.5*3.141592
        # self.MS_x = msg.data[3]
        # self.MS_y = msg.data[4]
        self.MS_psi = -msg.data[17] + 0.5*3.141592

        
        # print(self.MS_x, self.MS_y)
        # print(self.CD_x, self.CD_y)
