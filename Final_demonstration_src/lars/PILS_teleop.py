...
def yaw_unwrap(yaw_new, yaw_prev):
    ...
    """
    if yaw_prev is None:
        return yaw_new#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
class KeyboardControlNode(Node):
    def __init__(self):
        ...
        # 변경
        # self.actuator_pub = self.create_publisher(Float64MultiArray, '/USVControlType', 10)
        ...
        self.STEER_UI_MIN = -10000.0
        self.STEER_UI_MAX =  10000.0

        self.nn = 0.0

        # 실제 조향 라디안 범위: ±(pi*60/180) = ±pi/3
        self.STEER_RAD_MAX = math.pi * 30.0 / 180.0  # 1.0472...

        # 추력 범위
        self.THRUST_MIN = -2000.0
        self.THRUST_MAX = 2000.0

        # 키 입력 1회당 목표값 변화량(“목표값”에 반영)
        self.STEER_STEP_UI = 1000.0     # 좌/우 한번 누를 때 목표 조향(UI) 변화량
        self.THRUST_STEP = 100.0       # 위/아래 한번 누를 때 목표 추력 변화량

        # 조향 자동 복귀(매 타이머마다 목표 조향을 0쪽으로 끌어당김)
        self.STEER_RETURN_PER_TICK = 1000.0  # dt=0.1이면 초당 80 UI 정도로 중앙 복귀

        # ===== 변화율 제한 (rate limit) =====
        # 타이머(dt)마다 적용될 최대 변화량
        self.MAX_DSTEER_UI_PER_TICK = 2000.0   # dt=0.1 -> 초당 300 UI
        self.MAX_DTHRUST_PER_TICK   = 500.0  # dt=0.1 -> 초당 2000 thrust
    ...
    def apply_thruster_deadzone(self, cmd: float) -> float:
        """
        cmd: 키보드에서 만든 thrust 명령(연속값)
        return: deadzone을 점프한 실제 출력값
        규칙:
          - cmd == 0 -> 0
          - cmd  > 0 -> 최소 +1200
          - cmd  < 0 -> 최대 -1000 (즉, -1000 이하로 바로 점프)
        """
        if cmd == 0.0:
            return 0.0
        if cmd > 0.0:
            return max(cmd, 1200.0)
        else:
            return min(cmd, -1000.0)
    ...
    def timer_callback(self):
        with self.lock:
            ...
            # self.applied_thrust = self.apply_thruster_deadzone(self.applied_thrust)
            ...
            # 변경 (USVControlType 배열: [targetRPM, targetSteering, bow(0)])
            out = Float64MultiArray()

            target_rpm = float(self.applied_thrust)          # 0~5000 (네 로직 유지)
            target_steering = float(self.applied_steer_ui)   # -300~300 (DDS short로 넣기 좋음)
            out.data = [target_rpm, -target_steering, 0.0]
            self.actuator_pub.publish(out)


            # self._hb += 1
            # if self._hb % 20 == 0:  # dt=0.05 -> 1초마다
            #     self.get_logger().info(f"PUB /USVControlType: {out.data}")
        ...
        try:
            with self.lock:
                ...
                if key == keyboard.Key.up:
                    ...
                    if 0.0 < self.target_thrust < 1200.0:
                        self.target_thrust = 1200.0
                    
                elif key == keyboard.Key.down:
                    self.target_thrust -= self.THRUST_STEP
                
                    if -1000.0 < self.target_thrust < 0.0:
                        self.target_thrust = -1000.0
...
#         # ===== 변화율 제한 (rate limit) =====
#         # 타이머(dt)마다 적용될 최대 변화량
#         self.MAX_DSTEER_UI_PER_TICK = 10000.0   # dt=0.1 -> 초당 300 UI
...
#             # (deadzone 적용)
#             # thrust_cmd = float(self.applied_thrust)
#             # target_rpm = self.apply_thruster_deadzone(thrust_cmd)

#             # target_steering = float(self.applied_steer_ui)
#             # out.data = [target_rpm, target_steering, 0.0]
#             # self.actuator_pub.publish(out)
