...
private:
    ...
    void run()
    {
        ...
        const double dx = x_ - start_x;
        const double dy = y_ - start_y;
        ...
        const double beta_dot = gamma * (Delta*y_ep / denom);

        beta += beta_dot * dt_control_;

        // (선택) crab angle 추정치 제한 (작은 각도 가정 + 튜닝 안정성)
        beta = std::clamp(beta, -Q_x, Q_x); // 약 +-34deg (원하면 조절)
        beta = beta;
