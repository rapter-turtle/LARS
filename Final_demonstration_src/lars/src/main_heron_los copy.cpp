...
static constexpr double ship_state_x = 0.0;
static constexpr double ship_state_y = 0.0;
...
private:
    ...
    bool loadParamsFromTxt(const std::string& path)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            try {
                ...
                else if (key == "lateral_speed")     set_if(key, lateral_speed, v);


                else if (key == "wpt_x1")     set_if(key, wpt_x1, v);
                else if (key == "wpt_x2")     set_if(key, wpt_x2, v);
                else if (key == "wpt_x3")     set_if(key, wpt_x3, v);
                else if (key == "wpt_x4")     set_if(key, wpt_x4, v);


                else if (key == "wpt_y1")     set_if(key, wpt_y1, v);
                else if (key == "wpt_y2")     set_if(key, wpt_y2, v);
                else if (key == "wpt_y3")     set_if(key, wpt_y3, v);
                else if (key == "wpt_y4")     set_if(key, wpt_y4, v);

                else if (key == "acceptance_radius")     set_if(key, acceptance_radius, v);
    ...
    void run()
    {
        ...
        //// ALOS guidance ////
        // if (CD_y <= y_ - offset_y_ + 3 ){
        //     if (!second_phase && !stop_phase){
        //         second_phase = true;
        //         stop_phase = true;
        //     }
        // }
        ...
        // "[ALOS] startx=%.2f starty=%.3f distance=%.3f",
        // start_x, start_y, CD_y - y_ + offset_y_ - 3.0);
        ...
        if (!wpt_track){
            ...
            if (!go_to_wpt1_)
            {
                const double dist_to_dp = std::hypot(x_local - DP_x_rel, y_local - DP_y_rel);
        
                if (dist_to_dp <= dp_dist)
                {
                    if (!dp_inside_)
                    {
                        dp_inside_ = true;
                        dp_enter_time_ = now;   // now는 run()에서 이미 만든 rclcpp::Time
                    }
                    else
                    {
                        double held = (now - dp_enter_time_).seconds();
                        dur_time = held;
                        if (held >= duration_time)
                        {
                            go_to_wpt1_ = true;     // 이제부터 목표를 wpt1로 바꾼다
                            dp_inside_ = false;     // 타이머 리셋
                        }
                    }
                }
                else
                {
                    dp_inside_ = false; // 반경 벗어나면 타이머 리셋
                }
            }
            ...
            const double target_change = std::hypot(x_t - x_, y_t - y_);
            ...
            if (save_y_t != y_t){
                ...
                moving_y = y_;
                if (moving_y > 2.5 && !go_to_wpt1_){
                    moving_y = 2.5;
                }
                
            }

            if (std::sqrt((moving_y - y_t)*(moving_y - y_t)) < 0.1){
                arrive_check = true;
                moving_y = y_t;
            ...
            const double seg_dy = moving_y - start_y;
            const double seg_norm = std::hypot(seg_dx, seg_dy);
            pi_h = std::atan2(seg_dy, seg_dx);
            ...
            if (std::sqrt((x_ - x_t)*(x_ - x_t) + (y_ - y_t)*(y_ - y_t)) < acceptance_radius){
                cnt += 1;
            }
            if (cnt == 3){
                cnt = 0;
        ...
        double DD = std::atan2(y_ep, Delta);

        double psi_d = 0.0;

        if (!wpt_track){
            psi_d = uvr_filter::wrapToPi(pi_h - beta - DD - yaw_);
        }
        else{
            psi_d = uvr_filter::wrapToPi(pi_h - DD - yaw_);

            std::cout << "cnt : "<< cnt <<", distance : "<<std::sqrt((x_-x_t)*(x_-x_t) + (y_-y_t)*(y_-y_t)) <<std::endl;
            std::cout << x_t - x_ << " , "<<y_t - y_<<" , "<<psi_d <<std::endl;
        ...
        // F_ = -Xu*stream_speed/u_F -Xuu*stream_speed*stream_speed/u_F + Kp_F * dist + Ki_F * dist_int;
        delta_ = -Kp_steer * psi_d - Kd_steer*r_;// + Ki_steer*steer_int;
        // std::cout<<dist << " , "<< dist_int<<std::endl;
        ...
        if (wpt_track){
            ...
            if ( x_t < 11){
                std::cout << "  [ Approach / DP ] " <<std::endl;
                std::cout << " DP point : ("<< x_t<<", "<<y_t << ")"<<std::endl;
                std::cout << " Tracking point : ("<< x_t<<", "<<moving_y << ")"<<std::endl;
                std::cout << " USV state : ("<< x_<<", "<<y_ << ")"<<std::endl;
                // std::cout << "y_ : "<<y_<<", DP : "<<y_t << ", moving_y : "<<moving_y<<std::endl;
                // std::cout << "x_ : "<<x_<<", x_t : "<<x_t <<std::endl;
                
                std::cout << " DP time : " << dur_time<<std::endl;
            }
            else{
                std::cout << "  [ Recovery ] " <<std::endl;
                std::cout << " Recovery point : ("<< x_t<<", "<<y_t << ")"<<std::endl;
                // std::cout << "Tracking point : ("<< x_t<<", "<<moving_y << ")"<<std::endl;
                std::cout << " USV state : ("<< x_<<", "<<y_ << ")"<<std::endl;
                // std::cout << "y_ : "<<y_<<", DP : "<<y_t << ", moving_y : "<<moving_y<<std::endl;
                // std::cout << "x_ : "<<x_<<", x_t : "<<x_t <<std::endl;
            ...
            if (go_to_wpt1_ && std::hypot(x_local - wpt_x1, y_local - wpt_y1) < 0.8){
                F_ = 3.0;
            }
            else{
                F_ = -Xu*stream_speed/u_F -Xuu*stream_speed*stream_speed/u_F + Kp_F * dist + Ki_F * dist_int;
        ...
        if (delta_ > 3000*3.1415/180){
            delta_ = 3000*3.1415/180 - 0.2;
        }
        else if (delta_ < -3000*3.1415/180){
            delta_ = -3000*3.1415/180 + 0.2;
        }
        ...
        msg.data = {thrust_pwm, delta_pwm, 0.0, 0.0};
...
private:
    ...
    double moving_y = 0.0;
    double save_y_t = 0.0;
    bool arrive_check = false;

    bool wpt_track = false;
    ...
    // --- DP hold timer / switch ---
    bool dp_inside_ = false;
    bool go_to_wpt1_ = false;      // DP hold 완료 후, wpt1로 "이동 단계"
    rclcpp::Time dp_enter_time_;


    

    std::string param_txt_path_ = "/home/user/aura_ws/src/lars/config/mpc_params_los.txt";
