...
public:
    ...
    bool step(double dob_u, double dob_v, double dob_r, double now_sec, double& delta_out, double& F_out)
    {
        ...
        for (int j = 0; j <= N_; ++j)
        {
            ...
            {
                xr = wpt_x_ + j * dt_control_ * params_.misc.MS_speed * std::cos(lars_.MS_psi);
                yr = wpt_y_ + j * dt_control_ * params_.misc.MS_speed * std::sin(lars_.MS_psi);
                pr = wpt_psi_;
            }
            else
            {
                if (!reference_.empty())
                {
                    ...
                    // xr = reference_[k].x - params_.misc.offset_x;
                    // yr = reference_[k].y - params_.misc.offset_y;
                    // pr = reference_[k].yaw;
        ...
        // clamp (kept)
        if (F_ >= 15.0) F_ = 15.0 - 0.1;
        else if (F_ <= 0.0) F_ = 0.0 + 0.01;

        const double delta_max = 3000.0 * M_PI / 180.0;
        if (delta_ >  delta_max) delta_ =  delta_max - 0.2;
        if (delta_ < -delta_max) delta_ = -delta_max + 0.2;

        // if (lars_.lars_mode == 1){
        //     std::fprintf(stderr, "Try to docking");
        // }

        

        // final phase thrust logic (kept, simplified)
        if (!eight_traj_check_)
        {
            if (final_get_in_)
            {
                const double rx = x_ - lars_.CD_x - params_.misc.offset_x;
                const double ry = y_ - lars_.CD_y - params_.misc.offset_y;

                if (!mission_over_)
                {
                    if (std::sqrt(rx*rx + ry*ry) < params_.misc.final_end_radius)
                        mission_over_ = true;
                    else
                    {
                        F_ = 20.0;
                        delta_ = 0.0;
                        std::fprintf(stderr, "[Craddle dock!!!] \n");
                    }
                }
                else
                {
...
private:
    ...
    void ModeCheck()
    {
        ...
        if (!BRS_exist_ && lars_mode_ == 1.0)
        {
            ...
            wpt_x_ = - params_.misc.offset_x + lars_.CD_x + params_.misc.DP_x_rel * std::cos(lars_.CD_psi) - params_.misc.DP_y_rel * std::sin(lars_.CD_psi);
            wpt_y_ = - params_.misc.offset_y + lars_.CD_y + params_.misc.DP_y_rel * std::cos(lars_.CD_psi) + params_.misc.DP_x_rel * std::sin(lars_.CD_psi);
            wpt_psi_ = lars_.CD_psi + params_.misc.DP_psi_rel;
            // std::fprintf(stderr,
            //     "OOOUUUUUTTT \n");
            return;
        }
        ...
        {
            wpt_x_ = - params_.misc.offset_x + lars_.CD_x;
            wpt_y_ = - params_.misc.offset_y + lars_.CD_y;
            wpt_psi_ = lars_.CD_psi;
            // std::fprintf(stderr,
            //     "[Mode 1] \n");
        }
    ...
    void checkWaypointHoldAndSwitch()
    {
        ...
        // std::fprintf(stderr, "[dist] = %f, [time] = %f", dist, wpt_hold_elapsed_);
    }
