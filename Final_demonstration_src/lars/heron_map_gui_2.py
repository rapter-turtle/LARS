...
x_actual_min = 20.0 - 20.0
x_actual_max = 20.0 + 20.0
y_actual_min = 20.0 - 20.0
y_actual_max = 20.0 + 20.0
x_center = 20.0
y_center = 20.0
...
class SensorFusionEKF(Node):
    def __init__(self):
        ...
        self.red_line, = self.ax1.plot([], [], 'r-', linewidth=2, label='Custom Line')
        ...
        # 흰 배경
        self.ref_line_in, = self.ax1.plot([], [], 'g-', label='Reference')  
        self.ax1.set_facecolor('white')
        self.fig.patch.set_facecolor('white')  # 전체 figure도 흰색

        # 축 범위 고정 (추천: autoscale 원하면 이 부분 빼도 됨)
        ...
        scale = 1.0
        self.arrow_length = 0.8#3.0/(x_actual_max - x_actual_min)*self.map_width
        size = 1.5
        hullLength = 0.6  # Length of the hull
        ...
        # self.state_pub = self.create_publisher(Float64MultiArray, '/StatusType', 10)
        # self.lars_state_pub = self.create_publisher(Float64MultiArray, '/lars_state', 10)
        # self.imu_pub = self.create_publisher(Imu, '/imu/data', 10)
        # self.control_sub = self.create_subscription(Float64MultiArray, '/USVControlType', self.control_callback, 10)

        self.fig.tight_layout()  # axes 사이 간격을 적당히 벌려줍니다.
        # self.ekf_sub = self.create_subscription(Float64MultiArray, '/StatusType', self.ekf_callback, 10)
        # self.lars_state_sub = self.create_subscription(Float64MultiArray, '/lars_state', self.lars_state_callback, 10)
        # self.thrust_sub = self.create_subscription(Float64MultiArray, '/USVControlType', self.thrust_callback, 10)
        ...
        self.ekf_sub = self.create_subscription(
            Float64MultiArray, '/StatusType', self.ekf_callback, self.qos_best_effort)

        self.lars_state_sub = self.create_subscription(
            Float64MultiArray, '/lars_state', self.lars_state_callback, self.qos_best_effort)

        self.thrust_sub = self.create_subscription(
            Float64MultiArray, '/USVControlType', self.thrust_callback, self.qos_best_effort)

        self.waypoint_sub = self.create_subscription(
            Waypoint, "/waypoints", self.waypoints_callback, self.qos_best_effort)

        self.mpc_sub = self.create_subscription(
            MPCTraj, '/mpc_vis', self.mpc_callback, self.qos_best_effort)

        self.traj_sub = self.create_subscription(
            Float64MultiArray, '/mpc_traj', self.traj_callback, self.qos_best_effort)

        self.DOB_sub = self.create_subscription(
            Float64MultiArray, '/DOB', self.DOB_callback, self.qos_best_effort)
    ...
    def set_red_line(self, start, end):
        """start = (x1, y1), end = (x2, y2) in map coordinates"""
        self.red_line_start = start
        self.red_line_end = end
    ...
    def ekf_callback(self, msg):  # - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        ...
        self.x_map_sensor = self.x_sensor
        self.y_map_sensor = self.y_sensor
    ...
    def update_plot(self, frame):
        if len(self.time_data) > 1:
            ...
            self.pred_line.set_data(self.pred_x,self.pred_y)
                    ...
                    # (you probably don't want to call this every frame, but it's okay for test)
            self.set_red_line((7, 3), (42.5, 22))

            # 👉 Actually update red line data here
            if self.red_line_start is not None and self.red_line_end is not None:
                x1, y1 = self.red_line_start
                x2, y2 = self.red_line_end
                self.red_line.set_data([x1, x2], [y1, y2])
            ...
            self.ax6.set_ylim(-10000.0, 10000.0)
