...
private:
    ...
    bool loadParamsFromTxt(const std::string& path)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            try {
                ...
                else if (key == "dist_int")     set_if(key, dist_int, v);
                else if (key == "dist_int_max")     set_if(key, dist_int_max, v);
    ...
    bool updateBRSExist()
    {
        ...
        // =====================================
        double x_rel = x_ + hp * cos(yaw_) - (CD_x - cradle_x_rel) - offset_x_;
        double y_rel = y_ + hp * sin(yaw_) - CD_y - offset_y_;
    ...
    void run()
    {
        ...
        // LOS Guidance
        double wpt_x_abs = wpt_x + offset_x_;
        double wpt_y_abs = wpt_y + offset_y_;

        // ---------- 2) 거리/방위 계산 ----------
        if (abs(yaw_) < heading_thres){            
            wpt_x_abs = wpt_x + offset_x_;
            wpt_y_abs = wpt_y + offset_y_;
            LOS_x = wpt_x + offset_x_;
            LOS_y = y_;
        ...
        const double dx = wpt_x_abs - x_;
        const double dy = wpt_y_abs - y_;
        const double dist = dx;

        // LOS 목표 헤딩(베어링)
        const double psi_d = std::atan2(dy, dx);

        // ---------- 3) yaw wrap 함수(추가 함수/멤버 없이 run 안에서만 사용) ----------
        ...
        const double e_psi = wrapToPi(psi_d - yaw_);
        ...
        delta_ = -Kp_steer * e_psi;

        RCLCPP_INFO(this->get_logger(),
        "[MPC] F=%.2f Delta=%.2f", F_, delta_);
